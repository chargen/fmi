...
\solution{
	...
	\begin{figure}[ht]
	    \centering
	    \begin{tikzpicture}[v/.append style={node distance=3cm}]
		\node[v,label=above left:$u$] (u){};
		\node[v,right of=u,label=below right:$v$] (v){};
		\node[v,above of=v,label=above left:$w$] (w){};
		\node[v,right=of w,label=above right:$t$] (t){};
		\draw[line width=1.6pt,->,bend right=15] (u) to (v);
		\draw[line width=1.6pt,->,bend right=15] (v) to (w);
		\draw[line width=1.6pt,->,bend right=15] (w) to (u);
		\draw[line width=1.6pt,->,bend right=15] (w) to (t);
		\draw[line width=1.6pt,->,bend right=15] (v) to (u);
		\draw[line width=1.6pt,->,bend right=15] (w) to (v);
		\draw[line width=1.6pt,->,bend right=15] (u) to (w);
		\draw[line width=1.6pt,->,bend right=15] (t) to (w);
	    \end{tikzpicture}
	\end{figure}
    \end{enumerate}
    \item U=$\{u\}$ I(R)=$\{\}$
